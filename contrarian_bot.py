...
CROWD_THRESHOLD = 0.70   # 70% of bets in one direction
MAX_PRICE_MOVE = 3.0     # price moved less than 3pp despite the crowd
MIN_BETS = 15            # need enough bets to establish a crowd
MAX_SINGLE_BET = 150     # only count "small" bets (< 150 Mana)


def detect_signals() -> list[dict]:
    markets = fetch_binary_markets_flexible(n=50, min_pool=600, min_close_days=3)
    if not markets:
        return []
    ...
    return signals[:2]  # conservative: max 2 per run
