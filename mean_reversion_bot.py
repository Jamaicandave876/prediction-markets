...
# Thresholds
EXTREME_HIGH = 82  # prob above this = overbought
EXTREME_LOW = 18   # prob below this = oversold
MIN_POOL = 800     # higher liquidity requirement (extreme markets are riskier)
...
def detect_signals() -> list[dict]:
    markets = fetch_binary_markets_flexible(
        n=60,
        min_pool=MIN_POOL,
        min_age_hr=24,      # must be at least a day old
        min_close_days=14,  # not closing within 2 weeks
    )
    if not markets:
        return []
    ...
    return signals[:3]  # max 3 new entries per run
