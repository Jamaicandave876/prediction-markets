...
MIN_DRIFT = 1.0       # lower threshold — even small trends near close are meaningful
MIN_CONSISTENCY = 50   # relaxed to catch more signals
...
def detect_signals() -> list[dict]:
    ...
    markets = fetch_binary_markets_flexible(
        n=80,
        min_pool=200,       # lower pool OK for closing markets
        min_age_hr=24,      # must have existed a while
        min_close_days=0.5, # at least 12 hours left
        max_close_days=5,   # closing within 5 days
    )
    if not markets:
        return []
