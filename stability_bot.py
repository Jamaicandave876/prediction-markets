...
MAX_RANGE_PP = 4.0   # max price range to qualify as "stable"
MIN_BETS = 25        # need enough bets to confirm stability
BUY_YES_ABOVE = 55   # lean YES if prob above this
BUY_NO_BELOW = 45    # lean NO if prob below this
...
def detect_signals() -> list[dict]:
    markets = fetch_binary_markets_flexible(
        n=50,
        min_pool=600,
        min_age_hr=48,     # must be established
        min_close_days=7,  # not closing soon
    )
    if not markets:
        return []
